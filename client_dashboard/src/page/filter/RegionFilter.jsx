...
import { FilterRegion } from "../../components";
...
const RegionFilter = () => {
  ...
  useEffect(() => {
    const results = searchFilter.filter(
      (resp) => resp.region.toLowerCase() === filterInfo
    );
    setData(results);
  }, [filterInfo]);
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
      <Header category="Filter" title="REGIONS" />
      <FilterRegion filterValueSelected={onFilterValueSelected} />
      <GridComponent
        dataSource={data}
        width="auto"
        allowPaging
